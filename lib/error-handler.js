...
module.exports = function(err, req, res) {
  ...
  switch(true) {
  // case msg.includes('All Fields'): return res.status(401).send(`${ err.name }: ${ err.message }`);
  ...
  case msg.includes('Duplicate'): return res.status(409).send(`${ err.name }: ${ err.message }`);
