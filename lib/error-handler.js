'use strict';

const debug = require('debug')('cfgram:error-handler');
/* istanbul ignore next */
module.exports = function(err, req, res) {
  debug(`#error-handler: ${ err.message }`);

  let msg = err.message.toLowerCase();


  switch(true) {
  case msg.includes('All Fields'): return res.status(400).send(`${ err.name }: ${ err.message }`);
  case msg.includes('authorization failed'): return res.status(401).send(`${ err.name }: ${ err.message }`);
  // case msg.includes('Authorized'): return res.status(401).send(`${ err.name }: ${ err.message }`);
  case msg.includes('validation failed'): return res.status(400).send(`${ err.name }: ${ err.message }`);
  case msg.includes('Invalid Request'): return res.status(404).send(`${ err.name }: ${ err.message }`);
  case msg.includes('objectid failed'): return res.status(404).send(`${ err.name }: ${ err.message }`);
  case msg.includes('duplicate key error'): return res.status(409).send(`${ err.name }: ${ err.message }`);
  default: return res.status(500).send(`${ err.name }: ${ err.message }`);
  }
};
